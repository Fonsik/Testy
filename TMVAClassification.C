...
int TMVAClassification( TString myMethodList = "" )
{
   ...
   if (!gSystem->AccessPathName( fname )) {
      input1 = TFile::Open( fname );
      input2 = TFile::Open( fname ); // check if file in local directory exists
   }
   else {
      TFile::SetCacheFileDir(".");
      input1 = TFile::Open("~/Test/Testy/out_bkg.root", "CACHEREAD");
      input2 = TFile::Open("~/Test/Testy/out_sig.root", "CACHEREAD");
   }
   if (!input1||!input2) {
   ...
   TTree *background     = (TTree*)input1->Get("ntuple");  
   TTree *signalTree     = (TTree*)input2->Get("ntuple");
  ...
  dataloader->AddVariable( "pyp", "Varpyp", "units", 'F' );
