...
//   reader->AddSpectator( "spec2 := var1*3",   &spec2 );
   ...
   TH1F KerF("KerasF", "", 1000, 5, 40);
   TH1F KerPy("KerasPy", "", 1000, 5, 40);
  ...
  // theTree->SetBranchAddress( "pzm", &pzm );
   ...
   for (Long64_t ievt=0; ievt<theTree->GetEntries();ievt++) {
      ...
      if (Use["BDT"   ])
      {
         ...
         if (varBDT>0.05) {h_minvBDT2.Fill(minv);}
         if (varBDT>0.1) {h_minvBDT3.Fill(minv);}
      }
      ...
      if (Use["MLP"   ])
      {
         ...
         if (varMLP>0.7) {h_minvMLP1.Fill(minv);}
         if (varMLP>0.8) {h_minvMLP2.Fill(minv);}
         if (varMLP>0.85) {h_minvMLP3.Fill(minv);}
      }

      if (Use["Likelihood"   ])
      {
          Float_t varLike = reader->EvaluateMVA("Likelihood method");
          if (varLike>0.8)   {h_minvLike1.Fill(minv);}
          if (varLike>0.9) {h_minvLike2.Fill(minv);}
          if (varLike>0.95) {h_minvLike3.Fill(minv);}
      }

      if (Use["LikelihoodD"   ])
      {
          Float_t varLikeD = reader->EvaluateMVA("LikelihoodD method");
          if (varLikeD>0)  {h_minvLikeD.Fill(minv);}
      }

      if (Use["LikelihoodPCA"   ])
      {
          Float_t varPCA = reader->EvaluateMVA("LikelihoodPCA method");
          if (varPCA>0.95)    {h_minvLPCA.Fill(minv);}
      }
      

      //Float_t varKerF = reader->EvaluateMVA("Fisher method");
      Float_t varKerPy = reader->EvaluateMVA("PyKeras method");

      //if (varKerF>0.5)     {KerF.Fill(minv);}
      if (varKerPy>0.3)    {KerPy.Fill(minv);}
